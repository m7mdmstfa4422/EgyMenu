...
export default function Chicken() {
  const [dataChicken, setDataChicken] = useState([])
  ...
  useEffect(() => {
    let isMounted = true
    document.title = "Chicken Dishes - Café Menu"
  ...
  if (!dataChicken || dataChicken.length === 0) return <p className="text-center text-gray-500">لا توجد بيانات.</p>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 justify-center items-center gap-6 place-items-center">
      {dataChicken.map((item) => {
        ...
        return (
          ...
          >
            {/* Product Image */}
            <div className="relative mx-4 mt-4 flex h-60 overflow-hidden rounded-xl sm:h-72 md:h-80 bg-gradient-to-br from-[#A35416]/10 to-[#E29C50]/10" style={{ transition: "box-shadow 420ms ease" }}>
              <img
              ...
              <button
                ...
                className={
                  "wishlist-btn absolute top-3 right-3 rounded-full p-2 backdrop-blur-sm transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#E29C50] focus:ring-offset-2"
                }
              ...
              <div className="quick-view absolute bottom-8 left-1/2 transform -translate-x-1/2 opacity-0 translate-y-4">
                <button
                  ...
                  className="rounded-full bg-[#A35416] px-4 py-2 text-sm font-semibold text-white shadow-lg transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-[#E29C50] focus:ring-offset-2"
                >
                  Quick View
                </button>
              </div>
            ...
            <div className="mt-4 px-4 pb-4 sm:mt-6 sm:px-6 sm:pb-6">
              <div
                className="block focus:outline-none focus:ring-2 focus:ring-[#E29C50] focus:ring-offset-2 rounded-lg p-2 -m-2 cursor-pointer"
                onClick={() => navigate(`/product/${item?.idMeal}`)}
              >
                <h3 className="text-xl font-bold text-[#2B2B2B] transition-colors group-hover:text-[#A35416] sm:text-2xl">
                  {item?.strMeal}
                </h3>
                <p className="mt-1 text-sm text-[#E29C50] font-medium">Crispy & Juicy Perfection</p>
              </div>
              ...
              <div className="mt-3 flex items-center sm:mt-4">
                <div className="flex items-center">
                  {[...Array(5)].map((_, i) => (
                    <svg key={i} className="h-4 w-4 text-[#E29C50] sm:h-5 sm:w-5" fill="currentColor" viewBox="0 0 20 20">
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    </svg>
                  ))}
                  <span className="ml-2 rounded-full bg-[#F5C98C]/20 px-2 py-0.5 text-xs font-semibold text-[#A35416] sm:text-sm">
                    4.9
                  </span>
                </div>
              </div>
              ...
              <div className="mt-3 flex items-center justify-between sm:mt-4">
                <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:gap-2">
                  <span className="text-2xl font-bold text-[#A35416] sm:text-3xl">$19.99</span>
                  <span className="text-sm text-[#A35416] font-medium">600g</span>
                </div>
                <div className="text-sm text-[#E29C50] font-medium">🍗 Crispy Roast</div>
              </div>

              {/* Add to Cart */}
              <button className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-[#A35416] to-[#E29C50] px-4 py-3 text-base font-semibold text-white transition-all duration-300 hover:from-[#E29C50] hover:to-[#A35416] hover:shadow-lg active:scale-95 focus:outline-none focus:ring-2 focus:ring-[#E29C50] focus:ring-offset-2 sm:px-6 sm:py-2 sm:text-lg">
                <svg
      ...
      <style>{`
        ...
        /* Badge */
        .badge { background: linear-gradient(90deg,#A35416,#E29C50); transition: transform 320ms cubic-bezier(.2,.9,.2,1); }
        .product-card:hover .badge { transform: translateY(-4px); }

        /* Wishlist button smoothing and state */
        .wishlist-btn { background-color: rgba(255,255,255,0.95); color: #A35416; box-shadow: 0 6px 18px rgba(163,84,22,0.06); }
