...
export default function Drink() {
  ...
  useEffect(() => {
    let isMounted = true
    document.title = "Drink Specials - Café Menu"
    const getDrink = async () => {
      try {
        setLoading(true)
        setError(null)
        const { data } = await axios.get("https://www.thecocktaildb.com/api/json/v1/1/filter.php?c=Shake")
        if (isMounted) {
  ...
  if (!dataDrink || dataDrink.length === 0) return <p className="text-center text-gray-500">لا توجد بيانات.</p>
