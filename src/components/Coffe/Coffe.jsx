...
export default function Coffe() {
  ...
  useEffect(() => {
    let isMounted = true
    document.title = "Coffee & Tea - Café Menu"
    const getCoffee = async () => {
      try {
        setLoading(true)
        setError(null)
        const { data } = await axios.get("https://www.thecocktaildb.com/api/json/v1/1/filter.php?c=Coffee%20/%20Tea")
        if (isMounted) {
          setDataCoffee(data?.drinks || [])
  ...
  function addToFavorites(item) {
    const stored = JSON.parse(localStorage.getItem(STORAGE_KEY) || "[]")
    const idStr = String(item?.idDrink)
  ...
  if (!dataCoffee || dataCoffee.length === 0) return <p className="text-center text-gray-500">لا توجد بيانات.</p>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 justify-center items-center gap-6 place-items-center">
      {dataCoffee.map((item) => {
        ...
        return (
          ...
          >
            {/* Product Image */}
            <div className="relative mx-4 mt-4 flex h-60 overflow-hidden rounded-xl sm:h-72 md:h-80 bg-gradient-to-br from-[#3C2A21]/10 to-[#6B4F3A]/10" style={{ transition: "box-shadow 420ms ease" }}>
              <img
                className="product-image w-full object-cover"
                src={item?.strDrinkThumb || "/placeholder.svg"}
                alt={item?.strDrink}
                loading="lazy"
              ...
              <button
                ...
                className={
                  "wishlist-btn absolute top-3 right-3 rounded-full p-2 backdrop-blur-sm transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#A67856] focus:ring-offset-2"
                }
              ...
              <div className="quick-view absolute bottom-8 left-1/2 transform -translate-x-1/2 opacity-0 translate-y-4">
                <button
                  onClick={() => navigate(`/product/${item?.idDrink}`)}
                  className="rounded-full bg-[#3C2A21] px-4 py-2 text-sm font-semibold text-white shadow-lg transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-[#A67856] focus:ring-offset-2"
                >
                  Quick View
                </button>
              </div>
            ...
            <div className="mt-4 px-4 pb-4 sm:mt-6 sm:px-6 sm:pb-6">
              <div
                className="block focus:outline-none focus:ring-2 focus:ring-[#A67856] focus:ring-offset-2 rounded-lg p-2 -m-2 cursor-pointer"
                onClick={() => navigate(`/product/${item?.idDrink}`)}
              >
                <h3 className="text-xl font-bold text-[#2B2B2B] transition-colors group-hover:text-[#6B4F3A] sm:text-2xl">
                  {item?.strDrink}
                </h3>
                <p className="mt-1 text-sm text-[#A67856] font-medium">Freshly Ground Blend</p>
              </div>
              ...
              <div className="mt-3 flex items-center sm:mt-4">
                <div className="flex items-center">
                  {[...Array(5)].map((_, i) => (
                    <svg key={i} className="h-4 w-4 text-[#A67856] sm:h-5 sm:w-5" fill="currentColor" viewBox="0 0 20 20">
              ...
              <div className="mt-3 flex items-center justify-between sm:mt-4">
                <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:gap-2">
                  <span className="text-2xl font-bold text-[#3C2A21] sm:text-3xl">$14.99</span>
                  <span className="text-sm text-[#6B4F3A] font-medium">250g</span>
                </div>
                <div className="text-sm text-[#A67856] font-medium">☕ Arabica Blend</div>
              </div>
      ...
      <style>{`
        ...
        /* Badge */
        .badge { background: linear-gradient(90deg,#6B4F3A,#3C2A21); transition: transform 320ms cubic-bezier(.2,.9,.2,1); }
        .product-card:hover .badge { transform: translateY(-4px); }

        /* Wishlist button smoothing and state */
        .wishlist-btn { background-color: rgba(255,255,255,0.95); color: #3C2A21; box-shadow: 0 6px 18px rgba(60,42,33,0.06); }
